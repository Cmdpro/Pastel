...
//		int renameCost = 0; // We removed this line - Renames are free
        if (!inputStack.isEmpty() && EnchantmentHelper.canStoreEnchantments(inputStack)) {
            ...
            if (this.newItemName != null && !StringUtil.isBlank(this.newItemName)) {
                if (!this.newItemName.equals(inputStack
                                                 .getHoverName()
                                                 .getString())) {
...
//					 enchantmentLevelCost += renameCost;
                    if (inputStack.getHoverName() instanceof MutableComponent inputText && inputText
                                                                                               ...
                                                                                               .getColor() != null) {
                        outputStack.set(
                            DataComponents.CUSTOM_NAME, Component
                                .literal(this.newItemName)
                                .setStyle(Style.EMPTY.withColor(inputText
                                                                    .getStyle()
                                                                    .getColor()))
                        );
                    } else {
...
//				 enchantmentLevelCost += renameCost;
            ...
            if (pigmentInRepairSlot && text instanceof MutableComponent mutableText) {
                var newColor = ((PigmentItem) repairSlotStack.getItem())
                    .getInkColor()
                    .getColorInt();
                Component newName = mutableText.setStyle(mutableText
                                                             .getStyle()
                                                             .withColor(newColor));
