...
public class NectarLanceItem extends LightGreatswordItem implements SlotBackgroundEffectProvider {
    ...
    protected void applyLungeHitEffects(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (target.hasEffect(MobEffects.POISON)) {
            ...
            if (scaling > 0) {
                ...
                target.playSound(
                    PastelSoundEvents.DEEP_CRYSTAL_RING, 0.5F, 0.8F + target
                                                                          .getRandom()
                                                                          .nextFloat() * 0.4F
                );
            }
            ...
            if (stolenEffect.isEmpty() || !target.removeEffect(stolenEffect
                                                                   .get()
                                                                   .getEffect()))
                return;
            ...
            target.playSound(
                PastelSoundEvents.SOFT_HUM, 0.275F, 0.8F + target
                                                               .getRandom()
                                                               .nextFloat() * 0.4F
            );
        }
    ...
    private static void applyDoTProc(
        ...
        target.playSound(
            PastelSoundEvents.DEEP_CRYSTAL_RING, 1.25F, 0.9F + target
                                                                   .getRandom()
                                                                   .nextFloat() * 0.2F
        );
    }
