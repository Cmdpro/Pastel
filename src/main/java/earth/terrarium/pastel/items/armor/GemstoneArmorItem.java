...
public class GemstoneArmorItem extends ArmorItem implements ArmorWithHitEffect {
    ...
    public void onHit(ItemStack itemStack, DamageSource source, LivingEntity targetEntity, float amount) {
        ...
        targetEntity
            .level()
            .playSound(
                null, targetEntity.blockPosition(), SoundEvents.AMETHYST_BLOCK_HIT, SoundSource.PLAYERS, 1.0F, 1.0F);
        targetEntity
            .level()
            .playSound(
                null, targetEntity.blockPosition(), SoundEvents.SPLASH_POTION_BREAK, SoundSource.PLAYERS, 1.0F, 1.0F);
    ...
    public void addTooltip(List<Component> tooltip, @NotNull ArmorItem.Type equipmentSlot) {
        switch (equipmentSlot) {
            case HELMET -> tooltip.add(Component
                                           .translatable("item.pastel.fetchling_helmet.tooltip")
                                           .withStyle(ChatFormatting.GRAY));
            case CHESTPLATE -> tooltip.add(Component
                                               .translatable("item.pastel.ferocious_chestplate.tooltip")
                                               .withStyle(ChatFormatting.GRAY));
            case LEGGINGS -> tooltip.add(Component
                                             .translatable("item.pastel.sylph_leggings.tooltip")
                                             .withStyle(ChatFormatting.GRAY));
            case BOOTS -> tooltip.add(Component
                                          .translatable("item.pastel.oread_boots.tooltip")
                                          .withStyle(ChatFormatting.GRAY));
        }
    }
