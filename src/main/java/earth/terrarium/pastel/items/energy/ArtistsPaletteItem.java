...
public class ArtistsPaletteItem extends PastelTrinketItem
    implements InkStorageItem<TotalCappedElementalMixingInkStorage>, LoomPatternProvider, ExtendedItemBarProvider {
    ...
    @Override
    public ExtendedItemBarProvider.BarSignature getSignature(
        @Nullable Player player, @NotNull ItemStack stack, int index) {
        ...
        if (colors.size() == 1) {
            ...
            return new ExtendedItemBarProvider.BarSignature(
                1, 13, 14, progress, 1, color.getColorInt() | 0xFF000000, 2, DEFAULT_BACKGROUND_COLOR);
        }

        if (colors.isEmpty()) return new ExtendedItemBarProvider.BarSignature(
        ...
        return new ExtendedItemBarProvider.BarSignature(
            1, 13, 14, progress, 1, blendedColor, 2, DEFAULT_BACKGROUND_COLOR);
    }
