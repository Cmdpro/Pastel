...
public class InkDrainTrinketItem extends PastelTrinketItem
    implements InkStorageItem<FixedSingleInkStorage>, ExtendedItemBarProvider, SlotBackgroundEffectProvider {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltip, TooltipFlag type) {
        ...
        if (storedInk >= MAX_INK) {
            ...
            tooltip.add(Component
                            .translatable(
                                "pastel.tooltip.ink_drain.tooltip.ink_for_next_step", storedInk, inkStorage
                                    .getStoredColor()
                                    .getColoredInkName(), Support.getShortenedNumberString(nextStepInk - storedInk)
                            )
                            .withStyle(ChatFormatting.GRAY));
        }
    }
    ...
    public void setEnergyStorage(ItemStack itemStack, InkStorage storage) {
        ...
        itemStack.set(
            DataComponents.RARITY, storage.isFull() ? Rarity.EPIC : super
                .getDefaultInstance()
    ...
    public BarSignature getSignature(@Nullable Player player, @NotNull ItemStack stack, int index) {
        ...
        return new BarSignature(
            1, 13, 14, progress, 1, inkColor.getTextColorInt(), 2, ExtendedItemBarProvider.DEFAULT_BACKGROUND_COLOR);
    }
