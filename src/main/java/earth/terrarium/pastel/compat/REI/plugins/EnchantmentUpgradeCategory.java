...
public class EnchantmentUpgradeCategory extends EnchanterCategory<EnchantmentUpgradeDisplay> {
    ...
    public void setupWidgets(
        ...
        widgets.add(Widgets
                        .createButton(
                            new Rectangle(startPoint.x - 8 + 84, startPoint.y + 20, 8, 8), Component.literal("-"))
                        .onClick(b -> display.index = Math.clamp(display.index - 1, 1, max - 1))); // decrement
        widgets.add(Widgets
                        .createButton(
                            new Rectangle(startPoint.x - 8 + 94, startPoint.y + 20, 8, 8), Component.literal("+"))
        ...
        widgets.add(new IndexedEntryWidget(
            new Point(startPoint.x - 8 + 111, startPoint.y - 7 + 14), () -> display.index - 1)
                        .markInput()
                        .entries(inputs.get(overUnlocked ? EnchantmentUpgradeDisplay.OVERXP_INDEX
        ...
        // center input slot
        widgets.add(new IndexedEntryWidget(
            new Point(startPoint.x - 8 + 31, startPoint.y - 7 + 40), () -> display.index - 1)
                        .markInput()
                        .entries(inputs.get(overUnlocked ? EnchantmentUpgradeDisplay.OVERCHANT_INDEX
        ...
        var outputSlot = new IndexedEntryWidget(
            new Point(startPoint.x - 8 + 111, startPoint.y - 7 + 40), () -> display.index - 1)
            .markOutput()
            .disableBackground()
        ...
        var levelLabel = Widgets
            .createLabel(new Point(startPoint.x - 11 + 70, startPoint.y + 2), getDescriptionText(display))
            .leftAligned()
            .color(NORMAL_COLOR)
            .noShadow();
        ...
        var costLabel = Widgets
            .createLabel(new Point(startPoint.x - 11 + 70, startPoint.y - 11 + 85), getDescriptionText(display))
            .leftAligned()
            .color(NORMAL_COLOR)
            .noShadow();
