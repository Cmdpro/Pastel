...
public class PastelNodeBlockEntityRenderer implements BlockEntityRenderer<PastelNodeBlockEntity> {
    ...
    public void render(
        ...
        var color = ColorHelper.colorIntToVec(node.networkUUID
                                                  .flatMap(id -> Pastel
                                                      .getClientInstance()
                                                      .getNetwork(id))
        ...
        var innerRing = vertexConsumers.getBuffer(RenderType.entityTranslucent(node
                                                                                   .getInnerRing()
                                                                                   .map(
        ...
        var redstoneRing = vertexConsumers.getBuffer(RenderType.entityTranslucent(node
                                                                                      .getRedstoneRing()
                                                                                      .map(
        ...
        if (crystal.hasOuterRing()) {
            var outerRing = vertexConsumers.getBuffer(RenderType.entityTranslucent(node
                                                                                       .getOuterRing()
                                                                                       .map(
                                                                                           PastelUpgradeSignature::outerRing)
                                                                                       .orElse(OUTER_RING)));
        ...
        Minecraft
            ...
            .getItemRenderer()
            .renderStatic(
                crystal.crystal, ItemDisplayContext.NONE, LightTexture.FULL_BRIGHT, overlay, matrices, vertexConsumers,
                node.getLevel(), 0
            );
        matrices.popPose();
    }
