...
//	}
    ...
    public @Nullable BlockState handleFluidCollision(
        ...
        if (otherState.is(FluidTags.WATER)) {
            return state.isSource() ? PastelBlocks.FROSTBITE_CRYSTAL
                .get()
                .defaultBlockState() : Blocks.CALCITE.defaultBlockState();
        } else if (otherState.is(FluidTags.LAVA)) {
            return state.isSource() ? PastelBlocks.BLAZING_CRYSTAL
                .get()
                .defaultBlockState() : Blocks.COBBLED_DEEPSLATE.defaultBlockState();
