...
public class FabricationChestBlockEntityRenderer implements BlockEntityRenderer<FabricationChestBlockEntity> {
    ...
    public static @NotNull LayerDefinition getTexturedModelData() {
        ...
        root.addOrReplaceChild(
            "crafting_tablet", CubeListBuilder
                .create()
                .texOffs(48, 0)
                .addBox(-6.5F, 2.0F, -8.5F, 13.0F, 3.0F, 13.0F, new CubeDeformation(0.0F)),
            PartPose.offset(0.0F, -15.0F, 2.0F)
        );

        root.addOrReplaceChild(
            "assembly", CubeListBuilder
                .create()
                .texOffs(0, 0)
    ...
    public void render(
        ...
        BlockState blockState = bl ? chest.getBlockState() : PastelBlocks.FABRICATION_CHEST
            .get()
            .defaultBlockState()
            .setValue(ChestBlock.FACING, Direction.SOUTH);

        matrices.pushPose();
        float f = blockState.hasProperty(ChestBlock.FACING) ? blockState
        ...
        if (outputs.size() == 1) {
            Minecraft
                ...
                .getItemRenderer()
                .renderStatic(
                    null, outputs.getFirst(), ItemDisplayContext.GROUND, false, matrices, vertexConsumers, world, light,
                    overlay, 0
                );
        } else {
