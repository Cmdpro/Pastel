...
public interface InkPowered {
        ...
        if (canUseClient()) {
            if (getUsedColors().size() > 1) {
                tooltip.add(Component
                                .translatable("pastel.tooltip.ink_powered.prefix")
                ...
                tooltip.add(Component
                                .translatable(
                                    "pastel.tooltip.ink_powered.consume", getUsedColors()
                                        .get(0)
                                        .getColoredInkName()
                                )
                                .withStyle(ChatFormatting.GRAY));
    ...
    static long getAvailableInk(@NotNull Player player, InkColor color) {
        ...
        // trinket slots
        available += getCurioInkSlots(player)
            .mapToLong(stack -> tryGetEnergy(stack, color))
            .sum();
