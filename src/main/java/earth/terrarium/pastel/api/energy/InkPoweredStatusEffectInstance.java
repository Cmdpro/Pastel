...
public class InkPoweredStatusEffectInstance {
    ...
    public static final Codec<InkPoweredStatusEffectInstance> CODEC = RecordCodecBuilder.create(i -> i
        .group(
    ...
    public static void buildTooltip(
        ...
        if (!effects.isEmpty()) {
            ...
            for (InkPoweredStatusEffectInstance entry : effects) {
                ...
                mutableText.withStyle(effect
                                          .getEffect()
                                          .value()
                                          .getCategory()
                                          .getTooltipFormatting());
                mutableText.append(Component
                                       .translatable(
                                           "pastel.tooltip.ink_cost", Support.getShortenedNumberString(cost.cost()),
                                           cost
                                               .color()
                                               .getColoredInkName()
                                       )
            ...
            if (!attributeModifiers.isEmpty()) {
                ...
                for (var pair : attributeModifiers) {
                    var translatedAttribute = Component.translatable(pair
                                                                         .getA()
                                                                         .value()
                                                                         .getDescriptionId());
                    ...
                    if (statusEffect > 0.0D) {
                        tooltip.add((Component.translatable(
                            "attribute.modifier.plus." + mutableText
                                .operation()
                                .id(), ItemAttributeModifiers.ATTRIBUTE_MODIFIER_FORMAT.format(d), translatedAttribute
