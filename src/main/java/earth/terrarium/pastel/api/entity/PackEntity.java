...
public interface PackEntity<T extends Mob & PackEntity<T>> {
    ...
    class FollowClanLeaderGoal<E extends Mob & PackEntity<E>> extends Goal {
        ...
        protected int getSurroundingSearchDelay(E fish) {
            return reducedTickDelay(MIN_SEARCH_DELAY + fish
                                                           .getRandom()
        ...
        private void createNewPack() {
            List<E> possiblePackmates = this.entity
                .level()
                .getEntitiesOfClass(
                    (Class<E>) this.entity.getClass(), this.entity
                        .getBoundingBox()
                        .inflate(8.0, 8.0, 8.0),
                    (Predicate<LivingEntity>) livingEntity -> livingEntity instanceof PackEntity<?> packEntity &&
                                                              (packEntity.canHaveMoreInGroup() ||
                                                               !packEntity.hasLeader())
                );
