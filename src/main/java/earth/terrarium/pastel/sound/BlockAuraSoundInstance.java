...
public class BlockAuraSoundInstance extends AbstractSoundInstance implements TickableSoundInstance {
    ...
    public void tick() {
        ...
        float targetVolume = (float) Mth.clamp(
            (sources.size() * 0.05 - 0.5)
        ...
        var distance = Mth.clampedLerp(
            1, 0, Math.sqrt(Minecraft
                                .getInstance()
                                .getCameraEntity()
                                .distanceToSqr(absX, absY, absZ)) / MAX_DISTANCE
        );
        this.volume = (float) (volumeHold * distance * 0.8);

        double cameraEntityEyeY = Minecraft
            .getInstance()
            .getCameraEntity()
            .getEyeY();
        ...
        if (volumeHold > 0.25) {
            ...
            ParticleHelper.playTriangulatedParticle(
                world, PastelParticleTypes.AZURE_AURA, Support.chanceRound(chance * 3, random), true,
                new Vec3(24, 8, 24), -4, true, pos, new Vec3(0, 0.04D + random.nextDouble() * 0.06, 0)
            );
            ParticleHelper.playTriangulatedParticle(
                world, PastelParticleTypes.AZURE_MOTE_SMALL, Support.chanceRound(chance, random), false,
                new Vec3(16, 8, 16), -6, false, pos, Vec3.ZERO
            );
            ParticleHelper.playTriangulatedParticle(
                world, PastelParticleTypes.AZURE_MOTE, Support.chanceRound(chance, random), true, new Vec3(16, 6, 16),
                -4, false, pos, Vec3.ZERO
            );
        }
    }
    ...
    private void updatePositionAndCount() {
        ...
        for (BlockPos source : sources) {
            if (!world.hasChunkAt(source) || !world
                .getBlockState(source)
                .is(PastelBlockTags.AZURITE_ORES)) { // tag is hardcoded for now. But should we have more blocks like
                // that, we can easily split it
                toRemove.add(source);
            } else {
                x += source.getX();
                y += source.getY();
    ...
    public boolean isStopped() {
        ...
        if (volume <= 0) {
            ...
            Entity cameraEntity = Minecraft
                .getInstance()
                .getCameraEntity();
            done = cameraEntity == null || cameraEntity
                                               .position()
                                               .distanceToSqr(absX, absY, absZ) > MAX_DISTANCE * MAX_DISTANCE;
        }
    ...
    public static void addToExistingInstanceOrCreateNewOne(Level world, BlockPos pos) {
        ...
        if (nearestDistance < SPACING * SPACING * SPACING) {
            ...
            BlockAuraSoundInstance newInstance = new BlockAuraSoundInstance(
                PastelSoundEvents.OST_AZURE, world, pos.immutable());
