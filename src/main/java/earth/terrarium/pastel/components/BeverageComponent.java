...
public record BeverageComponent(long daysAged, int alcoholPercent, float thickness) implements TooltipProvider {
    ...
    public static final Codec<BeverageComponent> CODEC = RecordCodecBuilder.create(i -> i
        .group(
            Codec.LONG
        ...
        if (daysAged > 365) {
            ...
            if (ageInDays == 0)
                tooltip.accept(Component
                                   .translatable(
                                       "item.pastel.infused_beverage.tooltip.age_years", ageInYears, alcoholPercent)
