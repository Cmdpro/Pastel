...
public class DimensionRenderEffects {
    ...
    public static float interpTarget, interp, lastInterpTarget, fogTarget = 1F, fogDarkness = 1F,
        lastFogTarget = 1F, nearTarget = 1F, near = 1F, lastNearTarget = 1F, farTarget = 1F, far = 1F, lastFarTarget
        = 1F,
        redTarget, red, lastRedTarget, greenTarget, green, lastGreenTarget, blueTarget, blue, lastBlueTarget,
        blendTarget, blend, lastBlendTarget;
    ...
    public static void clientTick(ClientLevel world, Entity entity, Holder<Biome> biome) {
        ...
        if (shouldUpdate) {
        ...
        } else if (currentBiome == null || !currentBiome
            .unwrapKey()
            .equals(biome.unwrapKey())) {
            if (forceBiomeUpdate)
            ...
            if (GRADING_QUEUE.ready()) {
                GRADING_QUEUE.set(
                    COLOR_GRADING_DATA
                        .getOrDefault(biomeKey, DEFAULT)
                        .asArray(), ColorGrading.GRADING_OUT.clone()
                );
            } else {
                GRADING_QUEUE.accept(COLOR_GRADING_DATA
                                         .getOrDefault(biomeKey, DEFAULT)
                                         .asArray());
            }
        ...
        isInDarkenedBiome = DARKENING_MULTIPLIERS.containsKey(biome
                                                                  .unwrapKey()
                                                                  .orElse(null));
    ...
    public static float getInterp() {
        ...
        double y = lerp(
            client
                .getTimer()
                .getGameTimeDeltaPartialTick(false), client.cameraEntity.yOld, client.cameraEntity.getY()
        );
    ...
    public static float getDarknessInterpolation() {
        return lerp(
            Minecraft
                .getInstance()
                .getTimer()
                .getGameTimeDeltaPartialTick(false), (float) DimensionRenderEffects.darkenTicks,
            DimensionRenderEffects.lastDarkenTicks
        ) / INTERP_TICKS * getInterp();
    }
