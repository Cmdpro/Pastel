...
public enum PedestalRecipeTier implements StringRepresentable {
    ...
    ADVANCED(
        PastelAdvancements.Midgame.BUILD_ADVANCED_PEDESTAL_STRUCTURE, new GemstoneColor[]{
        BuiltinGemstoneColor.CYAN, BuiltinGemstoneColor.MAGENTA, BuiltinGemstoneColor.YELLOW, BuiltinGemstoneColor.BLACK
    }
    ),
    COMPLEX(PastelAdvancements.Lategame.BUILD_COMPLEX_PEDESTAL_STRUCTURE, BuiltinGemstoneColor.values());
        ...
        switch (this) {
            case COMPLEX -> {
                if (AdvancementHelper.hasAdvancement(
                    player, PastelAdvancements.Lategame.BUILD_COMPLEX_PEDESTAL_STRUCTURE_WITHOUT_MOONSTONE)) {
