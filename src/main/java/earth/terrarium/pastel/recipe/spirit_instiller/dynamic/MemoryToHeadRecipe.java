...
public class MemoryToHeadRecipe extends SpiritInstillerRecipe {
    ...
    public MemoryToHeadRecipe() {
        super(
            ...
            IngredientStack.ofItems(PastelBlocks.MEMORY
                                        .get()
                                        .asItem()), IngredientStack.ofItems(PastelItems.VEGETAL.get(), 4),
    ...
    public ItemStack assemble(InstanceRecipeInput<SpiritInstillerBlockEntity> recipeInput, HolderLookup.Provider drm) {
         ...
         * we spawn the entity from the memory, process its loot table with a custom damage type that guarantees a
         * head drop,
         ...
         */
        Optional<Entity> optionalEntity = MemoryBlockEntity.hatchEntity(
            world, pos, spiritInstillerBlockEntity.getItem(0));
        if (optionalEntity.isPresent()) {
            if (optionalEntity.get() instanceof LivingEntity livingEntity && world != null) {
                ...
                List<ItemStack> loot = lootTable.getRandomItems(
                    lootContextParameterSet, livingEntity.getLootTableSeed());
