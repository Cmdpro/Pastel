...
public class SpiritInstillerRecipe extends GatedStackPastelRecipe<InstanceRecipeInput<SpiritInstillerBlockEntity>> {
    ...
    public boolean matches(InstanceRecipeInput recipeInput, Level world) {
        ...
        if (recipeInput.size() > 2) {
            if (ingredientStacks
                .get(CENTER_INGREDIENT)
                .test(recipeInput.getItem(CENTER_INGREDIENT))) {
                if (ingredientStacks
                    .get(FIRST_INGREDIENT)
                    .test(recipeInput.getItem(FIRST_INGREDIENT))) {
                    return ingredientStacks
                        .get(SECOND_INGREDIENT)
                        .test(recipeInput.getItem(SECOND_INGREDIENT));
                } else if (ingredientStacks
                    .get(FIRST_INGREDIENT)
                    .test(recipeInput.getItem(SECOND_INGREDIENT))) {
                    return ingredientStacks
                        .get(SECOND_INGREDIENT)
    ...
    public ItemStack assemble(InstanceRecipeInput<SpiritInstillerBlockEntity> recipeInput, HolderLookup.Provider drm) {
        ...
        if (resultStack.is(PastelBlocks.MEMORY
                               .get()
                               .asItem())) {
    ...
    public static class Serializer implements RecipeSerializer<SpiritInstillerRecipe> {
        ...
        public static final MapCodec<SpiritInstillerRecipe> CODEC = RecordCodecBuilder.mapCodec(i -> i
            .group(
                Codec.STRING
